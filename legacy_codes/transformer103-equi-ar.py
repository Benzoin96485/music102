...
def train_main_loop(transformer, optim, trainset, validset, lr, n_epoch, patience):
    ...
    for ep in tqdm(range(n_epoch)):
        ...
        for idx, src, tgt in tqdm(trainset):
            optim.zero_grad()
        ...
        total_loss = 0
        with torch.no_grad():
            for idx, src, tgt in tqdm(validset):
                output = transformer(src, tgt[:, :-1, :])
                loss = criterion(output.contiguous().view(-1), tgt[:, 1:, :].contiguous().view(-1))
    ...
    with torch.no_grad():
        for idx, src, tgt in tqdm(testset, total=len(testset)):
            ...
            for t in range(1, tgt.size(1)):
                ...
                current_tgt = torch.cat([current_tgt, sampled_tgt.unsqueeze(1)], dim=1)
            x_gen = (current_tgt[:, 1:, :] >= rate).long()
...
eval_main_loop(transformer, "model_best_autoreg_equi.pt", testset, rate=0.5)