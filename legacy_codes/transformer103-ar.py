...
def eval_main_loop(transformer, checkpoint, testset):
    ...
    with torch.no_grad():
        for idx, src, tgt in tqdm(testset, total=len(testset)):
            if idx[0] != "853":
                continue
            current_tgt_enc = tgt[:, :1, :]
            for t in range(1, tgt.size(1)):
                output = transformer(src, current_tgt_enc, None, None).detach()
                tgt_enc_prediction = torch.bernoulli(output[:, -1, :])
                current_tgt_enc = torch.cat([current_tgt_enc, tgt_enc_prediction.unsqueeze(1)], dim=1)
            x_gen = current_tgt_enc
            x_gens.append((idx, x_gen))
            count += 1

    torch.save(x_gens, "song_test_music103_control.pt")
...
eval_main_loop(transformer, "model_best_autoreg.pt", testset)