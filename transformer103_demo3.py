...
class Transformer(nn.Module):
    def __init__(self, src_vocab_size, tgt_vocab_size, d_model, num_heads, num_layers, d_ff, max_seq_length, dropout):
        ...
        self.decoder_embedding = EmbedHead(tgt_vocab_size + 9, d_model, d_model, d_model)
        self.positional_encoding = PositionalEncoding(d_model)
        self.decoder_positional_encoding = DecoderPositionalEncoding(d_model)

        self.encoder_layers = nn.ModuleList([EncoderLayer(d_model, num_heads, d_ff, dropout) for _ in range(num_layers)])
        self.decoder_layers = nn.ModuleList([DecoderLayer(d_model, num_heads, d_ff, dropout) for _ in range(num_layers)])

        self.fc = nn.Linear(d_model, tgt_vocab_size + 9)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, src, tgt):
        ...
        tgt_embedded = self.dropout(self.decoder_positional_encoding(self.decoder_embedding(tgt), tgt))
        ...
        return torch.cat([torch.sigmoid(output[:,:,:12]), output[:,:,12:]], dim=-1)
...
if __name__ == "__main__":
    ...
    # load data
    if exists("new_trainset2.pkl") and exists("new_validset2.pkl") and exists("new_testset2.pkl"):
        print("splitted dataset found!")
        with open("new_trainset2.pkl", "rb") as f:
            trainset = pickle.load(f)
        with open("new_validset2.pkl", "rb") as f:
            validset = pickle.load(f)
        with open("new_testset2.pkl", "rb") as f:
    ...
    if mode == "train": 
        ...
        for epoch in tqdm(range(200)):
            for i, pair_data in tqdm(enumerate(trainset)):
                ...
                optimizer.zero_grad()
                # Use tgt_data as input to the transformer
        ...
        for i, pair_data in tqdm(enumerate(testset)):
            ...
            current_tgt = tgt_data[:, :1, :]  # Use the first step as a starting point

            for t in range(1, tgt_data.size(1)):  # Generate timestep by timestep
                # print(src_data.shape, current_tgt.shape)
                output = transformer(src_data, current_tgt).detach()
                #print(output.shape)
                probabilities = output[:, -1, 12:] # [batch, 1, 9]
                chord_prediction = output[:, -1, :12]  # Assuming other features are `tgt_data`

                #probabilities = torch.softmax(probabilities, dim=-1)  # First 12 values are probabilities
                chord_prediction = torch.bernoulli(chord_prediction)  # Randomly sample 0 or 1 based on probabilities
                length_class = torch.argmax(probabilities, dim=-1)
                onehot_encoding = nn.functional.one_hot(length_class, num_classes=9).float()

                # generated_tgt.append(next_tgt)
                token = torch.cat([chord_prediction, onehot_encoding], dim=-1)
                current_tgt = torch.cat([current_tgt, token.unsqueeze(1)], dim=1)
